...
- `RTKBinaryManager` 找不到当前平台 binary：middleware 不改写；Roc command tool 仅在 Roc shell 风险策略允许时按原路径执行。
- `rtk rewrite` 返回 unsupported：Roc command tool 仅在 Roc shell 风险策略允许时按原路径执行，并记录 `command_not_supported`。
