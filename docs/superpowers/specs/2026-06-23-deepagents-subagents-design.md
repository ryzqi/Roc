...
- 当前仓库安装的 `deepagents` 1.10.x 类型声明，尤其是 `CreateDeepAgentParams`、`SubAgent`、`AsyncSubAgent`、`SubagentRunStream` 和 `streamEvents(..., { version: 'v3' })` 的运行形态。
...
- `permissions` 只能收窄，不能扩大 `createRocFilesystemPermissions()`。
...
- AsyncSubAgent 的运行状态属于当前 DeepAgents thread/checkpointer state；如果需要 Roc 重启后可靠恢复，必须使用持久 checkpointer。`MemorySaver` 只能承诺进程内状态。
...
- 所有 emitted event 带 sequence，确保并发流最终能稳定回放。
