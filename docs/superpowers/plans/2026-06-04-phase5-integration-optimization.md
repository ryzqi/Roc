...
- [ ] **Step 2: Modify existing main entry**
...
Assert each current IPC domain delegates to the matching capability from Phase 2-3. Assert unknown capability fails with current `IpcResult` error shape.
...
Assert:
- first start with old database creates plugin data and migration run record.
...
Migration lock file path is `<userData>/plugin-data/.migration-lock`. A completed migration writes `<userData>/plugin-data/.migration-complete.json` with source checksum and plugin database checksums.
...
- [ ] **Step 1: Write optimization tests**
...
- [ ] **Step 2: Update electron-vite config**
