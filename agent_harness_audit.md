...
| `src/main/plugins/task/task-repository.ts` | verified | Task repository facade used by plugin and scheduler | Source read and changed | Added `recordBackgroundTaskStartFailure()` for AHA-005; AHA-007 now rejects non-positive scheduled-run limits before SQLite query | AHA-005/AHA-007 focused tests, `pnpm typecheck`, strict unused scan, `git diff --check` passed |
