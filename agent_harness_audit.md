...
| `src/main/services/deep-agent/store-memory-backend.ts` | located | Adapts DeepAgents StoreBackend to Roc memory file whitelist/security/capacity policy | Source read; `store-memory-backend.test.ts` read | Validates memory keys, filters read/search/list results, blocks upload/download, and validates write/edit final content | Focused tests read; not run this phase yet |
| `src/main/services/memory/store-slots.ts` | located | Defines memory virtual paths, namespaces, and workspace-required behavior | Source read; `store-slots.test.ts` read | Maps global/workspace slots and rejects workspace `USER.md` plus workspace memory without workspace | Focused tests read; not run this phase yet |
| `src/main/services/memory/sqlite-store.ts` | located | Durable LangGraph Store implementation used by DeepAgents StoreBackend | Source read; `sqlite-store.test.ts` read | Implements `BaseStore` over SQLite with namespace/key/value validation and basic search/list APIs | Focused tests read; not run this phase yet |
| `src/main/services/memory/security-scan.ts` | located | Blocks dangerous memory writes before persistence | Source read; `security-scan.test.ts` read | Scans prompt injection, credentials, SSH backdoors, and invisible Unicode; formats bounded issue summaries | Focused tests read; not run this phase yet |
| `src/main/services/memory/capacity.ts` | located | Enforces configured character limits per memory kind | Source read; `capacity.test.ts` read | Counts Unicode code points and returns formatted overflow guidance | Focused tests read; not run this phase yet |
