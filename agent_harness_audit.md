...
| `src/main/plugins/task/task-repository.ts` | verified | Task repository facade used by plugin and scheduler | Source read and changed | Added `recordBackgroundTaskStartFailure()` to atomically persist failed scheduled-start attempts and pause the task via existing failure behavior | AHA-005 focused tests, `pnpm typecheck`, strict unused scan, `git diff --check` passed |
...
| `src/main/plugins/task/task-repository-mappers.ts` | located | Row-to-shared-type mapper for task/background persistence | Source read | Maps background task capability JSON and task run capability JSON; `TaskThreadRow.kind` is typed narrowly as `ActiveTaskItem['kind']` despite DB supporting `TaskKind`, which is a type-level audit note pending follow-up classification | Not changed |
| `src/main/plugins/task/index.ts` | verified | Task plugin capability registration, scheduler wiring, and agent run event subscriptions | Source read | `runBackgroundNow` starts agent runs in existing task thread with persisted `workspacePath` and enabled capabilities; task plugin mirrors agent run lifecycle events into task repository | AHA-005 task/shared focused tests passed |
