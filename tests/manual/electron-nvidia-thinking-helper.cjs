...
function pickNvidiaConfig(document) {
  const providers = document && document.providers && document.providers.providers;
  if (!Array.isArray(providers)) {
    throw new Error('settings providers missing');
  }
  const provider = providers.find((entry) => entry && entry.id === 'nvidia' && entry.type === 'nvidia');
  if (!provider) {
    throw new Error('nvidia provider missing');
  }
  const model =
    provider.models.find((entry) => entry && entry.id === document.providers.defaultModelId && entry.enabled) ||
    provider.models.find((entry) => entry && entry.enabled);
  if (!model) {
...
function buildModelKwargs(provider, streaming) {
  const kwargs = {};
  if (typeof (provider.options && provider.options.thinking) === 'boolean') {
    kwargs.chat_template_kwargs = {
      thinking: provider.options.thinking
    };
  }
  if (typeof (provider.options && provider.options.includeReasoning) === 'boolean' && !streaming) {
...
async function run() {
  ...
  for await (const chunk of stream) {
    ...
    const additionalReasoning = chunk.additional_kwargs && chunk.additional_kwargs.reasoning_content;
    const blockReasoning = Array.isArray(chunk.contentBlocks)
      ? chunk.contentBlocks.filter((entry) => entry && entry.type === 'reasoning')
      : [];
  ...
  const rawSse = await collectRawSseSample({
    ...
    modelId: model.id,
    thinking: provider.options && typeof provider.options.thinking === 'boolean' ? provider.options.thinking : true
  });
  ...
  return {
    ...
    provider: {
      ...
      endpoint: provider.endpoint,
      modelId: model.id,
      thinking: provider.options && 'thinking' in provider.options ? provider.options.thinking : null,
      includeReasoning: provider.options && 'includeReasoning' in provider.options ? provider.options.includeReasoning : null
    },
    stream: {
      chunkCount,
      reasoningChunkCount,
